...
PlaceCalculator.prototype.CalculateWinningBets = function(bets, raceResult) {
	var selection = _.filter(bets, function(bet) {
		return bet.product.toUpperCase() === 'P' && PlaceMatch(bet, raceResult);
	});
	return _.groupBy(selection, function(bet) { return bet.selection; });	
}

function PlaceMatch (bet,raceResult) {
	return (bet.selection == raceResult.first || bet.selection == raceResult.second || bet.selection == raceResult.third);
}
